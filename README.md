...
- Ruta de montaje recomendada para emular el entorno local:
   - `C:/xampp/htdocs/PetsBook`
- Importar la copia de base de datos incluida en `database.sql` en su servidor.
- Configuración de conexión:
   - El archivo `conexion.php` utiliza las variables definidas en `config.local.php`.
   - En esta entrega, las cadenas de conexión de `config.local.php` se dejan vacías por seguridad y para evitar incompatibilidades entre entornos.
   - Tras importar la base de datos, completar en `config.local.php` los datos de su servidor (`host`, `port`, `name`, `user`, `pass`).
- Gestor de base de datos:
   - El gestor usado durante el desarrollo no afecta a la corrección, ya que la evaluación se realiza importando `database.sql` en el servidor de corrección.
